...
const Generator = () => {
    ...
    const [randomNumber, setRandomNumber] = useState(Math.floor(Math.random() * (quoteList.length-1)));

    const changeQuote = () => {
        const generateNumber = () => {
            let newNumber = Math.floor(Math.random() * (quoteList.length-1));
...
return generateNumber;
        ...
        const goodNumber = generateNumber();
        setRandomNumber(goodNumber);
    ...
    return (
        ...
        <Generate changeQuote={changeQuote}/>   
        <Quote randomNumber={randomNumber} quoteList={quoteList}/>
        <Tweet randomNumber={randomNumber} quoteList={quoteList}/>
